...
import com.kieronquinn.app.smartspacer.plugin.tasker.ui.screens.configuration.complication.picker.ComplicationPickerViewModel.State
import org.koin.androidx.viewmodel.ext.android.viewModel
import com.kieronquinn.app.shared.R as SharedR

class ComplicationPickerFragment: BaseSettingsFragment(), BackAvailable {

    companion object {
        private const val KEY_TARGET_PICKER = "complication_picker"
        private const val KEY_SMARTSPACER_ID = "smartspacer_id"

        fun Fragment.setupComplicationPickerListener(callback: (smartspacerId: String) -> Unit) {
    ...
    private val viewModel by viewModel<ComplicationPickerViewModel>()
    ...
    private fun State.Loaded.loadItems(): List<BaseSettingsItem> {
        return complications.map {
            Setting(
                it.name,
                getString(R.string.configuration_complication_select_subtitle, it.smartspacerId),
                icon = null
            ) {
                onComplicationClicked(it)
            }
        }.ifEmpty {
            listOf(
                Card(
                    ContextCompat.getDrawable(requireContext(), SharedR.drawable.ic_info),
                    getString(R.string.configuration_complication_select_empty)
                )
            )
        }
    }

    private fun onComplicationClicked(complication: Complication) {
        setFragmentResult(
            KEY_TARGET_PICKER,
            bundleOf(KEY_SMARTSPACER_ID to complication.smartspacerId)
        )
        viewModel.dismiss()
    }
