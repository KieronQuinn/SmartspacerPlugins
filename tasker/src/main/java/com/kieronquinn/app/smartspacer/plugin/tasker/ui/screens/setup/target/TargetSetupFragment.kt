...
import com.kieronquinn.app.smartspacer.plugin.tasker.ui.screens.setup.target.TargetSetupViewModel.State
...
class TargetSetupFragment: BaseSettingsFabFragment(), BackAvailable {
    ...
    companion object {
        ...
        internal val CONFIG_NAME = StringInputFragment.Config(
            "",
            REQUEST_KEY_NAME,
            R.string.target_setup_name_title,
            R.string.target_setup_name_content,
            R.string.target_setup_name_title,
            inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_FLAG_CAP_WORDS
        )
    }

    private val viewModel by viewModel<TargetSetupViewModel>()
    ...
    private fun State.Loaded.loadItems(): List<BaseSettingsItem> {
        val name = if(name.isNullOrEmpty()) {
            getString(R.string.target_setup_name_content)
        }else name
        return listOf(
            Card(
                ContextCompat.getDrawable(requireContext(), SharedR.drawable.ic_info),
                getString(R.string.target_setup_header)
            ),
            Setting(
                getString(R.string.target_setup_name_title),
                name,
                ContextCompat.getDrawable(requireContext(), R.drawable.ic_tasker),
                onClick = viewModel::onNameClicked
            ),
            Setting(
                getString(R.string.target_setup_name_id_title),
                smartspacerId,
